...
class ImageModel:
    def __init__(self):
        ...
        self.transformations: List[Callable[[Image.Image], Image.Image]] = []  # Holds pipeline transformations
    ...
    def add_transformation(self, transformation: Callable[[Image.Image], Image.Image]) -> None:
        """Add a transformation to the pipeline."""
        self.transformations.append(transformation)

    def apply_brightness(self, brightness_value: float) -> Callable[[Image.Image], Image.Image]:
        ...
        def transformation(image: Image.Image) -> Image.Image:
            ...
            if pixel_size > 1:
                ...
                return pixelated.resize(
                    (pixelated.size[0] * pixel_size, pixelated.size[1] * pixel_size), Image.NEAREST
                )
            return image
        return transformation
    ...
    def invert_colours(self) -> Callable[[Image.Image], Image.Image]:
        ...
        def transformation(image: Image.Image) -> Image.Image:
            if image.mode == "RGBA":
                ...
                inverted_image = ImageOps.invert(Image.merge("RGB", (r, g, b)))  # Invert RGB channels
                return Image.merge("RGBA", (inverted_image.split()[0], inverted_image.split()[1], inverted_image.split()[2], a))
            else:
